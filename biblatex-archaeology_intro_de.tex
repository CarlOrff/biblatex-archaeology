...
Wer \Biber im Toolmodus benutzen will, findet das Datenmodell und die Vererbung in der Datei \Datei{bibertool.xm_} und kann sie in seine eigene Konfigurationsdatei einsetzen wie in \cref{kap_biber} beschrieben. Da das Datenmodell von \archbib{} das von \blx{} nur erweitert, aber nichts wegnimmt, kann man damit gefahrlos auch  Daten für die Standardstile bearbeiten.

\subsection{Sortierung}

\subsection{Dokumententypen}
\paragraph{\Typ{inreference}}
\Typ{inreference} ist in \blx{} nur ein Alias für \Typ{incollection}. In \archbib{}hat dieser Datentyp eine eigenständige Bedeutung, weil man im Titel einen Ausdruck wie \enquote{s.\,v.} (\Latein{sub voce}) voranstellen kann.
...
Manche Stile verlangen in bestimmten Fällen Ligaturen der Initialen der Vornamen (St, Ph, Th, Ch). Die \blx-Autoren haben eine entsprechenden Wunsch einmal abgelehnt, weil ihnen das zu randständig erschien. Der klassische Weg, so etwas in \BibTeX{} zu erreichen, ist die folgende Namensschreibung: \begin{lstlisting}
author = {Müller, {Th}\relax omas}
\end{lstlisting}
 Das hat den Vorteil, daß es sortiert wird, aber den Nachteil, daß man es händisch in die Datenbanken eintragen muß. \archbib{} hat eine Option \Option{giveninitligatures}, der man eine Liste solcher Ligaturen übergeben kann, die dann bis zum ersten Treffer automatisch abgearbeitet wird. Allerdings geschieht dies auf der \LaTeX-Seite, wenn die Sortierung schon stattgefunden hat. T(rude) Meier müßte hier alphabetisch vor Th(orsten) Meier kommen, \Biber{} sortiert aber nach den vollständigen Vornamen. Solche Fälle sind selten und können mit dem \Feld{sortname}-Feld leicht repariert werden. Man muß aber beim Korrekturlesen wissen, daß so etwas passieren kann.

\paragraph{Trunkierung von Listen}
Wenn die Option \Option{preservelastauthor} wahr ist, dann werden Mamenslisten wie \Kbd{Müller AND Meier AND Schneider AND Schulze} statt mit \enquote{Müller, Meier u.\,a.} mit \enquote{Müller \textellipsis{} Schulze} trunkiert. Die Werte für \Option{maxnames} und \Option{minnames} gelten genauso.
...
\paragraph{Verlage}
Verlage werden von \archbib-Stilen normalerweise nicht genutzt. Trotzdem sollte man sie in der Datenbank angeben, weil es manchmal nützlich ist, auf ihr Vorhandensein zu testen. Im Gegensatz zu \blx{} sind Verlage sogar im Typ \Typ{thesis} erlaubt. Das hängt damit zusammen, daß manche Stile bei Hochschulschriften so etwas wie \enquote{ungedruckte Diss.} verlangen. Niederländische Dissertationen z.\,B. erscheinen aber schon vor dem Rigorosum als Verlagsausgabe. Das kann man durch so einen Test erkennen.
