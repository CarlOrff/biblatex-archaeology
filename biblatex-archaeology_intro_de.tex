...
% The table option is passed to xcolor package when loaded through hyperref.
...
%%% Table layout.
%\usepackage{colortbl}  % zebra striped tables (loads )
...
Nicht alle Dokumententypen, die \blx definiert, werden von den Standardstilen genutzt. Ich beschränke mich hier in \cref{tab:doctypes} auf die tatsächlich unterstützten. Wer spezielle Stile verwendet, wird in den Handbüchern Erklärungen zu den Erweiterungen finden.
...
\newcommand{\DokTypTabKopf}{
\rowcolor{white}
\multicolumn{1}{c}{\textbf{Dokumententyp}} &
\multicolumn{1}{c}{\textbf{Bedeutung}} &
\multicolumn{1}{c}{\textbf{Weitere Hinweise}} \\}

\begin{longtable}{p{0.24\textwidth}p{0.38\textwidth}p{0.38\textwidth}}
\caption[Dokumententypen in \blx]{Dokumententypen in \blx}
...
\rowcolor{white}\multicolumn{3}{r}{{\itshape Fortsetzung auf der nächsten Seite}}
\endfoot

\hline
\endlastfoot

\Typ{article} & Artikel in Zeitschriften &  \\
\Typ{book} & Monographie (kein Sammelwerk) &  \\
\Typ{mvbook} & wie \Typ{book}, aber mehrbändig &  \\
\Typ{inbook} & Teil von \Typ{book} oder \Typ{mvbook} mit eigenem Titel & z.\,B., wenn eine Monographie einen Aufsatz eines anderen Autors enthält \\
\Typ{bookinbook} & ein ursprünglich selbstständiges Buch, das jetzt ein einem anderen \Typ{book} oder \Typ{mvbook} wieder abgedruckt ist & Soetwas kann z.\,B. in Quellensammlungen vorkommen \\
\Typ{suppbook} & Supplement in \Typ{book} oder \Typ{mvbook} &  \\
\Typ{booklet} & wie \Typ{book}, aber ohne Verlag &  \\
\Typ{collection} & Sammelwerk & Vorsicht: keine Werke mit Veranstaltungstiteln (Konferenzen, Ausstellungen) \\
\Typ{mvcollection} & wie \Typ{collection}, aber mehrbändig &  \\
\Typ{incollection} & Aufsatz in \Typ{collection} oder \Typ{mvcollection} &  \\
\Typ{suppcollection} & Supplement in \Typ{collection} oder \Typ{mvcollection} &  \\
\Typ{manual} & Handbücher bzw. Dokumentationen für Geräte, Software, Laborausstattung etc. &  \\
\Typ{misc} & wenn nichts anderes paßt &  \\
\Typ{online} & Onlineressourcen &  alles, was mit URL zitiert werden muß \\
\Typ{patent} & Patentschriften &  \\
\Typ{periodical} & Zeitschriftenheft & nur, wenn man das ganze Heft als solches zitieren will \\
\Typ{suppperiodical} & Supplement in einer Zeitschrift &  \\
\Typ{proceedings} & Sammelwerk, das aus Veranstaltung hervorgegangen ist & der wesentliche Unterschied zu \Typ{collection} ist, daß man die ausrichtende Organisation angeben kann \\
\Typ{mvproceedings} & wie \Typ{proceedings}, aber mehrbändig &  \\
\Typ{inproceedings} & Aufsatz in \Typ{proceedings} oder \Typ{mvproceedings} &  \\
\Typ{reference} & Enzyklopädien, Wörterbücher, Lexika &  \\
\Typ{mvreference} & wie \Typ{reference}, aber mehrbändig &  \\
\Typ{inreference} & Eintrag in \Typ{reference} oder \Typ{mvreference} &  \\
\Typ{report} & Technische oder Forschungsberichte, Whitepapers & hier sollten der Typ und die herausgebende Organisation angeben werden\\
\Typ{set} & Liste von Datenbankeinträgen & ist hauptsächlich für numerische Stile gedacht \\
\Typ{thesis} & Examensschrift & hier sollten der Typ und die prüfende Hochschule angeben werden \\
\Typ{xdata} & ein Container, um die Daten an andere Einträge zu vererben & nicht zum eigenständigen Zitieren gedacht! \\
\end{longtable}
\subsection{Die Felder}
\BibTeX{} kennt verpflichtende und optionale Felder für jeden Dokumententyp. Das sind gewissermaßen die Felder, deren Auswertung von den Stilen garantiert wird. Praktisch spielt das keine große Rolle, denn grundsätzlich ist alles ein legitimer Datenbankeintrag, was ein \Feld{title}-Feld hat. Man kann davon ausgehen, daß jemand, der einen wissenschaftlichen Text mit \blx schreibt, weiß, welche Angaben für ein vollständiges Zitat eines Werkes gebraucht werden. Für Einzelheiten ist die Datei \Datei{biblatex-archaeology-example.bib} zu betrachten. In \blx gibt es auch kommaseparierte Listen (CSV), z. B. die Felder \Feld{keyword} und \Feld{option}.
...
\end{Verbatim}
\blx prüft jetzt, ob es sich bei dem Inhalt des Feldes um einen bekannten Schlüssel handelt und druckt bei Erfolg die landessprachliche Bezeichnung stattdessen aus. Das funktioniert auch mit Sprachen (\Feld{language}, \Feld{origlanguage}), Rollen (\Feld{authortype}, \Feld{editortype} etc.) und XXX .
...
Wie jedes Kulturaggregat sind auch Zitierstile historisch bedingt. Welchen Sinn z. B. ergibt die Angabe von Verlagsorten in einer Welt, in der Warenbestellungen kaum noch mit der gelben Post aufgegeben werden? Bemerkenswert hingegen ist die große Zurückhaltung bei den digitalen Identifikationsschlüsseln (ISBN/ISSN, DOI, URN etc.), deren nachhaltige Auflösbarkeit von den Bibliotheken gesichert werden muß. 
