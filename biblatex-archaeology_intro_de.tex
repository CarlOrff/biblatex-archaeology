...
\begin{xltabular}{\linewidth}{p{\ErsteSpalte}p{\ZweiteSpalte}L}
\rowcolor{zebraodd}
...
\Feld{crossref} & Schlüssel & Der Verweis eines Kindeintrages auf seinen Elterneintrag \\
\Feld{entryset } & Schlüsselliste & Einträge eines \Typ{set} \\
\Feld{execute} & Text & ausführbarer \LaTeX-code \\
\Feld{gender} & Muster & Genus und Numerus von Pronomen (\Kbd{sf}, \Kbd{sm}, \Kbd{sn}, \Kbd{pf}, \Kbd{pm}, \Kbd{pn} oder \Kbd{pp}) \\
...
\Feld{keywords} & CSV & Schlagwörter \\
\Feld{options} & CSV & Liste von \Kbd{key=value}-Ausdrücken für eintragsspezifische Optionen \\
\Feld{presort} & Text & Ausdruck zur Vorsortierung (damit kann man Gruppen in der Sortierung erzeugen) \\
\Feld{related} & Schlüsselliste & Liste von Einträgen, die in einer näher zu bestimmenden relation zum aktuellen Eintrag stehen \\
...
\Feld{sortname} & Name & ein alternativer Name für die Sortierung \\
\Feld{sortshorthand} & Text & wie \Feld{sortkey}, aber für die abgekürzt zitierte Literatur \\
\Feld{sorttitle} & ein alternativer Titel für die Sortierung &  \\
\Feld{sortyear} & Ganzzahl & ein alternatives Jahr für die Sortierung \\
\Feld{xdata} & Schlüsselliste & Verweise auf \Typ{xdata}-Container \\
