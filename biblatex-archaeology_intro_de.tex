...
\subsection{Funktionen von \Biber}
...
$ biber --tool-config
\end{lstlisting}
verrät, wo die aktuell in Gebrauch befindliche \Datei{bibertool.conf} abgelegt ist. Machen Sie davon eine Kopie, tauschen Sie in dieser die \Kbd{<datamodel>...</datamodel>}-Gruppe aus, entfernen Sie die Präfixe für den BCF-Namensraum (\Kbd{bcf:}). Dasselbe müssen sie für die \Kbd{<bcf:inheritance>...</bcf:inheritance>}-Gruppe tun, die die Feldnamen für die Vererbungsziele angibt, und evtl. für die \Kbd{<bcf:sourcemap>...</bcf:sourcemap>}-Gruppe, die Anweisungen für Textersetzungen wie z.\,B. die Umbennenung von Feldern enthält. Wenn die Zieldatei sortiert sein soll, übernehmen Sie auch die \Kbd{<bcf:sortingtemplate>...</bcf:sortingtemplate>}-Gruppe. Dann rufen Sie Biber im Toolmodus künfig mit der Option \Option{-g}\Kbd{=/path/to/bibertool-copy.conf} auf. Das Sortieren von Quelldatenbanken ist aber im Allgemeinen nicht sinnvoll.

\begin{lstlisting}
$ biber --tool --output-encoding=ascii <file>.bib
\end{lstlisting}
Das erstellt aus einer UTF-8-codierten Datenbank eine \Datei{<file>_bibertool.bib}, in der alle Zeichen höher als US-ASCII in \LaTeX-Makros umgewandelt sind, soweit \Biber  dafür Makros kennt -- unbekannte Zeichen werden markiert. Für den umgekehrten Vorgang gibt man \Kbd{UTF-8} an (oder nur \Option{-U}). Es gibt auch noch weitere Encodings, die man auch mit \Option{--input_encoding} angeben kann, falls man eine Datenbank konvertieren will. Zeichen, für die Biber kein Mapping im Zielformat kennt, werden dabei markiert. Es gibt noch eine ganze Reihe von Optionen, mit denen man Datenbanken formatieren, validieren und sortieren kann.
...
\printbibliography[keyword=source,title={Verzeichnis der Quellen]
...
\chapter{\archbib}\label{kap_blx{} arch}
