...
% arara: biber
% arara: lualatex
\documentclass[11pt,table]{scrreprt}
% The 'table' option is passed to xcolor package when loaded implicitly.
...
\newcommand{\SortierungDemo}[1]{\nocite{intro_sort_1,intro_sort_2,intro_sort_3,intro_sort_4,intro_sort_5,intro_sort_6,intro_sort_7,intro_sort_8,intro_sort_9,intro_sort_10,intro_sort_12,intro_sort_13,intro_sort_14,intro_sort_15,intro_sort_16,intro_sort_17,intro_sort_18,intro_sort_19,intro_sort_20,intro_sort_21,intro_sort_22,intro_sort_23,intro_sort_24,intro_sort_25,intro_sort_26,intro_sort_27,intro_sort_28,intro_sort_29,intro_sort_30}}
...
%%% and to refer to them with labels (fig. 3 or tab. 9).
\usepackage{float}
...
\subsection{Weitere Funktionen von \Biber}
...
\end{Verbatim}
Das erstellt jetzt eine Datenbank \Datei{<jobname>.bib}. So man möchte, kann man dabei mit der Option \Option{--output-resolve} die Vererbung von Daten zwischen Einträgen auflösen.

Wenn Biber mit der \Datei{<jobname>.bcf} aufgerufen wird, erhält es auch die in der Datei \Datei{<style>.dbx} gespeicherten Informationen über Dokumententypen und Felder. Z.\,B. müssen Datumsfelder, Listen und Namen geparst werden, damit ihre Elemente separat aufrufbar sind. Im Datenmodell steht, welche Felder das sind. Nun gibt es aber auch Szenarien, in denen man die Fähigkeiten von \Biber nutzen will, ohne daß ein Dokument verarbeitet wird. Dafür gibt es den sogenannten Werkzeugmodus, den man mit der Option \Option{--tool} aufruft.

\begin{Verbatim}
$ biber --tool --output-encoding=ascii <file>.bib
\end{Verbatim}
Das erstellt aus einer UTF-8-codierten Datenbank eine \Datei{<file>_bibertool.bib}, in der alle Zeichen jenseit US-ASCII in \LaTeX-Makros umgewandelt sind, soweit \Biber  dafür Makros kennt -- unbekannte Zeichen werden markiert. Für den umgekehrten Vorgang gibt man \Kbd{UTF-8} an (oder nur \Option{-U}). Es gibt auch noch weitere Encodings, die man auch mit \Option{--input_encoding} angeben kann, falls man eine Datenbank konvertieren will.
...
\subsection{Charakteristik des RGK-Stils}
...
\subsection{Stile}
\subsubsection{Autor/Jahr}
\subsubsection{Fußnoten}
\subsubsection{Numerisch}

\subsection{Kein \BibTeX  und die Konfiguration von \Biber}
...
Gleich, welche Methode benutzt wird, man wird sie nach jedem Update von \Biber wiederholen müssen.
...
\paragraph{\Kbd{digital} in Feld \Feld{entrysubtype}}
\paragraph{\Kbd{news} in Feld \Feld{entrysubtype}}

\subsection{Felder}

\subsection{Publikationen aus Veranstaltungen}
